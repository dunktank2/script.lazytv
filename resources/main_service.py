                    ...
                    if x["label"] == showtitle:
                        ...
                        if "episodes" in tmp_eps:
                            for y in tmp_eps["episodes"]:
                                if (
                                    fix_SE(y["season"]) == season_np
                                    and fix_SE(y["episode"]) == episode_np
                                ):
                                    ep_npid = y["episodeid"]
            ...
            if self.ep_details["item"]["type"] in ["unknown", "episode"]:
                ...
                try:
                    ...
                    if self.ep_details["item"]["episode"] < 0:
                        ...
                        prevcheck, show_npid, ep_npid = iStream_fix(
                            show_npid, showtitle, episode_np, season_np
                        )
                ...
                if prevcheck and show_npid not in randos and self.pl_running != "true":
                    ...
                    stored_seas = fix_SE(
                        int(WINDOW.getProperty("%s.%s.Season" % ("LazyTV", show_npid)))
                    )
                    stored_epis = fix_SE(
                        int(WINDOW.getProperty("%s.%s.Episode" % ("LazyTV", show_npid)))
                    )
                ...
                if resume_partials and self.ep_details["item"]["resume"]["position"] > 0:
                    seek_point = int(
                        (
                            float(self.ep_details["item"]["resume"]["position"])
                            / float(self.ep_details["item"]["resume"]["total"])
                        )
                        * 100
                    )
                    ...
                    if "type" in self.ndata["item"]:
                        ...
                        if self.ndata["item"]["type"] == "episode":
                            ...
                            if self.ndata["playcount"] == 1:
                                ...
                                tmp_showid = json_query(ep_to_show_query, True)["episodedetails"][
                                    "tvshowid"
                                ]
                                ...
                                if proceed:
                                    ...
                                    LazyPlayer.playing_showid = json_query(ep_to_show_query, True)[
                                        "episodedetails"
                                    ]["tvshowid"]
                                    log(
                                        "monitor supplied showid - "
                                        + str(LazyPlayer.playing_showid)
                                    )
...
class Main(object):
    ...
    def _daemon_check(self):
        ...
        if LazyPlayer.playing_showid and LazyPlayer.playing_showid in Main.nepl:
            ...
            tmp_uwep = max(
                ...
                int(
                    WINDOW.getProperty(
                        "%s.%s.CountUnwatchedEps" % ("LazyTV", self.sp_next)
                    ).replace("''", "0")
                )
                - 1,
            )
                ...
                if npodlist:
                    ...
                    if LazyPlayer.playing_epid not in npodlist:
                        ...
                        if cp != len(npodlist) - 1:
                            ...
                            self.np_next = npodlist[
                                cp + 1
                            ]  # if the episode is in the list then take the next item and store in temp
                            newod = [int(x) for x in npodlist[cp + 1 :]]

                            self.store_next_ep(self.np_next, "temp", newod, offd, tmp_uwep, tmp_wep)

                            log(
                                "supplied epid not last in list, retrieved new ep = "
                                + str(self.np_next)
                            )
                            ...
                            log(
                                "supplied epid in last position in odlist, flag to remove from nepl"
                            )
                            self.eject = (
                                True
                            )  # if the episode is the last in the list then send the message to remove the showid from nepl
            ...
            log(
                message="%s:%d" % (int(Main.target / 60 if Main.target else 0), Main.target % 60),
                label="target",
            )
        ...
        if Main.target:
            ...
            if (
                self.count == 0
            ):  # check the position of the playing item every 5 seconds, if it is past the Main.target then run the swap
    ...
    def reshuffle_randos(self, sup_rand=[]):
        ...
        for rando in shuf_rand:
            ...
            tmp_wep = WINDOW.getProperty("%s.%s.CountWatchedEps" % ("LazyTV", rando)).replace(
                "''", "0"
            )
            tmp_uwep = WINDOW.getProperty("%s.%s.CountUnwatchedEps" % ("LazyTV", rando)).replace(
                "''", "0"
            )
    ...
    def get_eps(self, showids=[]):
        ...
        if (
            "tvshows" not in self.lshowsR
        ):  # if 'tvshows' isnt in the result, have the list be empty so we return without doing anything
            self.show_lw = []
        else:
            self.show_lw = [
                x["tvshowid"] for x in self.lshowsR["tvshows"] if x["tvshowid"] in self.showids
            ]
        ...
        for my_showid in self.show_lw:  # process the list of shows
            ...
            for ep in self.eps:
                if ep["playcount"] != 0:
                    watched_showcount += 1
                    if (ep["season"] == Season and ep["episode"] > Episode) or ep[
                        "season"
                    ] > Season:
                        Season = ep["season"]
                        Episode = ep["episode"]
            ...
            unordered_ondeck_eps = [
                x
                for x in all_unplayed
                if x["season"] > Season or (x["season"] == Season and x["episode"] > Episode)
            ]
            ...
            self.count_uweps = (
                self.count_eps - self.count_weps
            )  # the total number of unwatched episodes
            ...
            if unordered_ondeck_eps:
                unordered_ordered_eps = sorted(
                    unordered_ondeck_eps,
                    key=lambda unordered_ondeck_eps: (
                        unordered_ondeck_eps["season"],
                        unordered_ondeck_eps["episode"],
                    ),
                )
            else:
            ...
            if (
                not ondeck_eps and not offdeck_eps
            ):  # ignores show if there is no on-deck or offdeck episodes
        ...
        if not monitor.abortRequested():
            ...
            if "episodedetails" in ep_details:  # continue only if there are details
                ...
                if ep_details["resume"]["position"] and ep_details["resume"]["total"]:
                    ...
                    played = "%s%%" % int(
                        (
                            float(ep_details["resume"]["position"])
                            / float(ep_details["resume"]["total"])
                        )
                        * 100
                    )
                else:
                ...
                WINDOW.setProperty(
                    "%s.%s.TVshowTitle" % ("LazyTV", TVShowID_), ep_details["showtitle"]
                )
                WINDOW.setProperty("%s.%s.Art(thumb)" % ("LazyTV", TVShowID_), art.get("thumb", ""))
                WINDOW.setProperty(
                    "%s.%s.Art(tvshow.poster)" % ("LazyTV", TVShowID_), art.get("tvshow.poster", "")
                )
                ...
                WINDOW.setProperty(
                    "%s.%s.CountonDeckEps" % ("LazyTV", TVShowID_), str(len(ondecklist))
                )
                ...
                WINDOW.setProperty(
                    "%s.%s.Art(tvshow.fanart)" % ("LazyTV", TVShowID_), art.get("tvshow.fanart", "")
                )
    ...
    def swap_over(self, TVShowID_):
        ...
        WINDOW.setProperty(
            "%s.%s.Title" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.Title" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
            "%s.%s.Episode" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.Episode" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
            "%s.%s.EpisodeNo" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.EpisodeNo" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
            "%s.%s.Season" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.Season" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
            "%s.%s.TVshowTitle" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.TVshowTitle" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
            "%s.%s.Art(thumb)" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.Art(thumb)" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
            "%s.%s.Art(tvshow.poster)" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.Art(tvshow.poster)" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
            "%s.%s.Resume" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.Resume" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
            "%s.%s.PercentPlayed" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.PercentPlayed" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
            "%s.%s.CountWatchedEps" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.CountWatchedEps" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
            "%s.%s.CountUnwatchedEps" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.CountUnwatchedEps" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
            "%s.%s.CountonDeckEps" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.CountonDeckEps" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
            "%s.%s.EpisodeID" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.EpisodeID" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
            "%s.%s.odlist" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.odlist" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
            "%s.%s.offlist" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.offlist" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
            "%s.%s.File" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.File" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
            "%s.%s.Art(tvshow.fanart)" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.Art(tvshow.fanart)" % ("LazyTV", "temp")),
        )
        ...
        WINDOW.setProperty(
            "%s.%s.Premiered" % ("LazyTV", TVShowID_),
            WINDOW.getProperty("%s.%s.Premiered" % ("LazyTV", "temp")),
        )
        WINDOW.setProperty(
